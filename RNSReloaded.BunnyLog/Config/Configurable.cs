...
// Generic hot-reload base class copied from the DamageTracker pattern (single source of truth
// for Reloaded II's config conventions in this repo). Kept self-contained so BunnyLog has no
// runtime dependency on DamageTracker.
    ...
    private static T TryGetValue<T>(Func<T> getValue, int timeout, int sleepTime, CancellationToken token = default)
        ...
        while (watch.ElapsedMilliseconds < timeout) {
            ...
            try {
                ...
                // file might still be being written; retry
            }
